...
add_action('init','jr_indeed_init', 10);
...
function jr_indeed_results( $is_ajax = false, $page = 1 ) {	
	...
	$params = array (
		'source'	  => 'Indeed',
		'title' 	  => __('Sponsored Job Listings', 'appthemes'),
...
function jr_indeed_search_results( $is_ajax = false, $page = 1 ) {
	...
	$params = array (
		'source'	  => 'Indeed',
		'title' 	  => __('Sponsored Search Results', 'appthemes'),
		'jobs_by_img' => 'http://www.indeed.com/p/jobsearch.gif',		
...
if ( get_option('jr_indeed_all_listings') == 'yes' ):
	add_action('before_jobs_taxonomy','jr_indeed_html_placeholder', 10, 2);	
	add_action('before_jobs_by_date','jr_indeed_html_placeholder', 10, 2);	
endif;	

if ( get_option('jr_indeed_front_page') == 'yes' ):	
	add_action('before_front_page_jobs','jr_indeed_html_placeholder');
endif;
...
?>
	...
	<p><?php _e('Fetching Indeed Job Listings','appthemes'); ?></p></div>	
<?php
