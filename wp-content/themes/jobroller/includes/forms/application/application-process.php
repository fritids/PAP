...
function jr_process_application_form() {
	...
	if (isset($_POST['apply_to_job'])) :
		...
		$required = array(
			'your_name' => __('Name', 'appthemes'),
			'your_message' => __('Message', 'appthemes')
		);
		...
		if (isset($_FILES['your_cv']) && !empty($_FILES['your_cv']['name'])) {
			...
			if (!in_array($extension, $allowed)) $errors->add('submit_error', __('<strong>ERROR</strong>: Only pdf, zip, doc, txt and rtf files are allowed.', 'appthemes'));
		}
		...
		if ($errors && sizeof($errors)>0 && $errors->get_error_code()) {
			...
			if ($errors && sizeof($errors)>0 && $errors->get_error_code()) {} else {
				...
				appthemes_display_notice( 'success', __('Your application has been sent successfully.', 'appthemes') );
