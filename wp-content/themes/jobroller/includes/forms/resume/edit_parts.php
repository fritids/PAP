...
function jr_edit_resume_parts($post) {
	...
	<div style="display:none">
		...
		<form id="websites" action="<?php echo get_permalink($resume); ?>" class="submit_form main_form modal_form" method="post">
			<h2><?php _e('Add Website', 'appthemes'); ?></h2>
			<p><?php _e('Add a website below to add it to your resume e.g. your portfolio or a Twitter account.', 'appthemes'); ?></p>
			
			<p><label for="website_name"><?php _e('Website Name', 'appthemes'); ?></label> <input type="text" class="text" name="website_name" id="website_name" /></p>
			<p><label for="website_url"><?php _e('Website URL', 'appthemes'); ?></label> <input type="text" class="text" name="website_url" id="website_url" /></p>
			
			<p><input type="submit" class="submit" name="save_website" value="<?php _e('Add', 'appthemes'); ?>" /></p>
		</form>
		...
		/* <![CDATA[ */
			...
			jQuery('input[name=save_website]').click(function(){
				...
				if (!web_name || !web_url) {
					jQuery('#fancybox-content form h2').after('<ul class="errors"><li><?php _e('Name and URL are required fields.', 'appthemes'); ?></li></ul>');
					return false;
				}
			});
...
function jr_process_resume_parts() {
	...
	if (isset($_POST['save_website'])) :
		...
		$message = __('Website Added', 'appthemes');
	...
	elseif (isset($_GET['delete_website']) && is_numeric($_GET['delete_website'])) :
		...
		$message = __('Website successfully deleted', 'appthemes');
