				 ...
				 WHERE  
				 	( meta_key = 'jr_alert' 
				 	 AND ( 
				 			...
				 			meta_value REGEXP '{$job_alert->alert_type}' 						
				 	 	 ) 				 	  	 
				 	 AND (
...
function jr_job_alerts_get_templates( $template_pattern = '*tmpl-*.html' ) {
	...
	foreach (glob($template_pattern) as $filename) {
		$tmpl = array ( basename( $filename ) => __('External HTML Template ('.basename( $filename ).')', 'appthemes') ); 		
...
?><rss version="2.0"
	...
	<channel>
		<title><?php bloginfo_rss('name'); echo __(' / Job Alerts Feed for ','appthemes') . $user->user_login; ?></title>
