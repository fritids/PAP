...
function jr_process_job_seeker_alerts_form() {	
	global $posted, $message, $wpdb, $user_ID, $app_abbr;
	...
	if ( !empty($_POST['save_alerts']) ) :	
		...
		if ( !$valid_alert ):
			...
			$errors->add('submit_error', __('<strong>ERROR</strong>: ', 'appthemes').__('You haven\'t specified any jobs alerts criteria.', 'appthemes'));
		...
		if ($errors && sizeof($errors)>0 && $errors->get_error_code()) { $posted['errors'] = $errors; } else {
			...
			update_user_meta( $user_ID, $app_abbr.'_alert_status', $posted['alert_status']);

			$message = __('Job Alerts Preferences Saved', 'appthemes');
		}	
